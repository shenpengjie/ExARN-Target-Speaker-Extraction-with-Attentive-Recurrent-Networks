...
from components.arn_wrapper import arn_block_separate_add
...
class NET_Wrapper(nn.Module):
    def __init__(self, frame_size, frame_shift, feature_dim, hidden_dim,infer=False, causal=True):
        ...
        self.arn_block1 = arn_block_separate_add(self.feature_dim, infer=self.infer, causal=self.causal)
        self.arn_block2 = arn_block_separate_add(self.feature_dim, infer=self.infer, causal=self.causal)
        self.arn_block3 = arn_block_separate_add(self.feature_dim, infer=self.infer, causal=self.causal)
        self.arn_block4 = arn_block_separate_add(self.feature_dim, infer=self.infer, causal=self.causal)
